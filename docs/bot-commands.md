...
| `@prowl-review configure <key=value …>` | Set per-PR review settings (`minSeverity`, `maxFindings`, `verify`); `configure reset` clears them. |
...
`@prowl-review configure minSeverity=major` sets review settings for the current
PR only; they persist in the summary's state marker and win over the repo config.
Invalid input replies with usage instead of silently weakening the review.
`@prowl-review configure reset` clears them.
